    ...
    while not q.empty():
        ...
        if v == -1:
            ...
            for e_u in g.incident(v):
                ...
                if toSave[u] == True and g.vs[u]["color"] != SAVED:
                    g.vs[u]["color"] = SAVED
...
def main(argv):
    ...
    try:
        ...
        print('view_instance.py -i <inputfile_name> -o <outputfile_name>.mp4')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('view_instance.py -i <inputfile_name> -o <outputfile_name>.mp4')
    ...
    # lista de vértices para serem salvos
    toSave = [False for v in range(0, n)]
    for v in input_f.readline().split():
        toSave[int(v)] = True
